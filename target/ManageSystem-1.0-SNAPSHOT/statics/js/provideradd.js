...
$(function(){
	...
	proCode.on("blur",function(){
		if(proCode.val() != null && proCode.val() != ""){
			...
			validateTip(proCode.next(),{"color":"red"},imgNo+" ID can not be null, please re-enter",false);
		}
	}).on("focus",function(){
		validateTip(proCode.next(),{"color":"#666666"},"* Please enter supplier ID",false);
	}).focus();
	
	proName.on("focus",function(){
		validateTip(proName.next(),{"color":"#666666"},"* Please enter supplier name",false);
	}).on("blur",function(){
		if(proName.val() != null && proName.val() != ""){
			validateTip(proName.next(),{"color":"green"},imgYes,true);
		}else{
			validateTip(proName.next(),{"color":"red"},imgNo+" Supplier name can not be null, please re-enter",false);
		}
	...
	proPhone.on("focus",function(){
		...
		if(proPhone.val().match(patrn)){
			...
			validateTip(proPhone.next(),{"color":"red"},imgNo + " Phone number format incorrect",false);
		}
	});
